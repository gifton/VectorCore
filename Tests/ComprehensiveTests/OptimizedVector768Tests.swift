...
struct OptimizedVector768Suite {
    ...
    @Test func testInitFromArray_SuccessAndThrow() {
        ...
        do { _ = try Vector768Optimized(bad); Issue.record("Expected dimensionMismatch not thrown") } catch let e as VectorError { #expect(e.kind == .dimensionMismatch) } catch { Issue.record("Unexpected error: \(error)") }
    }
